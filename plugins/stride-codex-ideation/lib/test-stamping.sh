...
# (skills/stride-ideation-stridify/SKILL.md Step 6 + Step 8b), but the two
# failure-mode behaviors the AC calls out are
...
# lowercase hex so the drift check (lib/drift_check.py) compares byte-for-byte. Validate that
