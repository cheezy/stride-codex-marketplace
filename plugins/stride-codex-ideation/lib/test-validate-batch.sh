...
# expected outcome: zero exit for structurally valid docs (advisory
# scored-field warnings on stderr are tolerated), zero exit + a stderr
# warning substring for the advisory pass, and non-zero exit + a matching
# error substring for structural or varchar(255) length violations.
#
# Run:
...
assert_warns_with() {
  ...
  # substring (the scored-field completeness pass writes warnings to stderr).
  local label="$1"
  local fixture="$2"
  local needle="$3"
  local stderr
...
# Python's len() matches. Length-sensitive fixtures are generated with
# json.dumps to avoid heredoc newline contamination; each carries the five
# scored fields so the length pass — not the advisory pass — is under test.
...
assert_silent "(f) boundary: exactly 255 characters passes silently" \
...
assert_silent "(f) multibyte: 255 CJK code points (765 UTF-8 bytes) passes — code points, not bytes" \
...
EOF
assert_silent "advisory: all five scored fields populated — validator is completely silent" \
  "$TMP/all_fields_silent.json"

# Ordering pin: a fatal check must exit BEFORE any advisory warning prints.
# This fixture is both structurally invalid (self/forward dep) and missing
# every scored field; the fatal exit must win and NO "warning:" line appears.
...
EOF
ORDERING_STDERR="$(python3 "$VALIDATOR" "$TMP/fatal_beats_warning.json" 2>&1 >/dev/null)"
ORDERING_EXIT=$?
if [ "$ORDERING_EXIT" -ne 0 ] && ! printf '%s' "$ORDERING_STDERR" | grep -Fq "warning:"; then
  PASS=$(( PASS + 1 ))
  printf 'PASS  advisory: warnings never precede a fatal failure (no warning on fatal exit)\n'
else
  FAIL=$(( FAIL + 1 ))
...
for repo_fixture in "$SCRIPT_DIR"/../fixtures/*-stride-batch.json; do
  assert_ok "repo fixture is structurally valid and within length bounds: $(basename "$repo_fixture")" \
    "$repo_fixture"
done
