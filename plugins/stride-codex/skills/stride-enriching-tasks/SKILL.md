...
`POST /api/tasks` takes a **request envelope**: the enriched task fields go under the `task` root key, and `agent_name` rides at the **top level, beside it** — set to `"Codex CLI"`, the exact same plain agent name sent as `agent_name` on claim and complete. A bare task object is rejected with `422 Missing 'task' key`.
...
```json
{
  ...
  "behaviour_test_matrix": [
    {
      "category": "Happy path",
      "behaviour": "Each comment renders its creation timestamp in the human-readable format",
      "test_name": "test/kanban_web/live/task_live/view_component_test.exs — \"renders a human-readable comment timestamp\"",
      "type": "unit",
      "status": "planned",
      "position": 0
    },
    {
      "category": "Boundary",
      "behaviour": "A comment created just before midnight renders the correct local date, not the UTC one",
      "test_name": "test/kanban_web/live/task_live/view_component_test.exs — \"renders the local date across a midnight boundary\"",
      "type": "unit",
      "status": "planned",
      "position": 1
    },
    {
      ...
      "behaviour": "The original bug no longer reproduces — a comment with a timestamp renders it rather than omitting it",
      "test_name": "test/kanban_web/live/task_live/view_component_test.exs — \"regression: comment timestamps are not dropped\"",
      "type": "integration",
      "status": "planned",
      "position": 2
    },
    {
      "category": "Null / empty",
      "behaviour": "A comment with no recorded timestamp renders without raising and omits the timestamp element",
      "test_name": "test/kanban_web/live/task_live/view_component_test.exs — \"renders a comment with no timestamp\"",
      "type": "unit",
      "status": "planned",
      "position": 3
    },
    {
      ...
      "na_reason": "The change adds no write path, so there is no concurrent-writer race to cover",
      "position": 4
    },
    {
      "category": "Lifecycle / wiring",
      "behaviour": "The timestamp is present on first render and survives a LiveView re-render after a new comment is added",
      "test_name": "test/kanban_web/live/task_live/view_component_test.exs — \"keeps timestamps across a re-render\"",
      "type": "integration",
      "status": "planned",
      "position": 5
    },
    {
      ...
      "behaviour": "The stored UTC timestamp round-trips through the formatter into the displayed string",
      "test_name": "test/kanban/tasks_test.exs — \"formats a stored UTC comment timestamp\"",
      "type": "unit",
      "status": "planned",
      "position": 6
    }
