...
In Codex CLI, hooks are executed **manually** — the agent runs each `.stride.md` command itself via the `shell` tool and captures the raw stdout/stderr. So your input is almost always:

1. **Raw text output** from manual hook execution — requiring full parsing (the primary Codex path)
2. **Structured JSON** — only if your environment wraps hook output through an automatic hook bridge (a `stride-hook` script). Codex's manual execution model does not produce this by default.
...
- **Raw text:** Input is plain text containing mixed tool output (test results, credo warnings, etc.). This is the normal Codex case.
- **Structured JSON:** Input contains a JSON object with `"hook"`, `"status": "failed"`, `"failed_command"`, `"stdout"`, `"stderr"`, `"commands_completed"`, and `"commands_remaining"` fields — present only when an automatic hook bridge produced the output.

### Parsing Raw Text Input (primary)

When you receive raw text — the normal Codex case — use the multi-tool output parsing strategy in the "Multi-Tool Output Parsing" section below to identify tool boundaries and parse each section with the appropriate pattern from the catalog.

### Parsing Structured JSON Input (when a hook bridge provides it)

If your environment wrapped the hook output as structured JSON, extract fields directly — no boundary detection needed:
...
4. Note `commands_remaining` — these did not run yet; after fixing the failure, re-running the hook will retry all commands including these
...
### When input is raw text (primary Codex path):
...
### When input is structured JSON (a hook bridge provided it):

Include the command sequence context:
