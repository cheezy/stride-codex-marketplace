<#
.SYNOPSIS
    Install Stride exploratory-testing skills and agents for Codex CLI.

.DESCRIPTION
    Installs Stride exploratory-testing skills (skills/) and agents (agents/) for
...
.EXAMPLE
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/cheezy/stride-codex-exploratory-testing/main/install.ps1))) -Project
...
if ($Project) {
    ...
    Write-Host "Installing Stride exploratory testing for Codex CLI into .agents\ (project-local)..."
}
else {
    $InstallDir = Join-Path $env:USERPROFILE '.agents'
    Write-Host "Installing Stride exploratory testing for Codex CLI into ~\.agents\ (global)..."
}
...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("stride-codex-exploratory-testing-" + [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Force -Path $tempRoot | Out-Null
$cloneDir = Join-Path $tempRoot 'stride-codex-exploratory-testing'
...
try {
    ...
    if ($Project) {
        ...
        Write-Host ""
        Write-Host "Note: Copy AGENTS.md to each project that uses Stride exploratory testing:"
...
Write-Host ""
Write-Host "Stride exploratory testing for Codex CLI installed successfully!"
Write-Host ""
...
Write-Host "  1. Point the plugin at a running, non-production app you are authorized to test"
Write-Host "  2. See the README for chartering, exploring, and debriefing sessions"
